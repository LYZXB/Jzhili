...
public class SolarLunarUtils {
	...
	/**
		...
		17-20： 表示闰月是大月(1111)30还是小月(0000)29，仅当存在闰月的情况下有意义。 
	 */
    // 计算阴历日期参照1900年到2049年
    ...
    // 对应的日期中文表现
    private final static String[] MONTH_STR = {
    		"*","初一","初二","初三","初四","初五",  
            "初六","初七","初八","初九","初十",  
            "十一","十二","十三","十四","十五",  
            "十六","十七","十八","十九","二十",  
    ...
    // 对应的天干数
    private final static String[] TIANGAN = {
    		"甲","乙","丙","丁","戊",
    		"己","庚","辛","壬","癸"
    };
    
    // 对应的地支数
    private final static String[] DIZHI = {
    ...
    private static String getTianGanYear(Integer year) {
    ...
    /*
		...
		还有戊癸何方觅，甲定之上好追求。
     */
    private static String getTianGanMonth(Integer year) {
    	int tianganYear = (year-4)%10%5;
    	int tianganMonth = 2*(tianganYear+1);
    	tianganMonth = tianganMonth > 9 ? 0 : tianganMonth;
    	return TIANGAN[tianganMonth];
    }
    private static String getDizhiYear(Integer year) {
    	return DIZHI[(year-3)%12];
    }
    private static String getDizhiMonth(Integer month) {
    	return DIZHI[month+1];
    }
     ...
     * @return 阴历日期 yyyy-MM-dd 1(1闰月0非闰月)
     * @throws Exception
     * @author liu 2015-1-5
     */
    public static String solarToLunar(String solarDate) throws Exception{
        ...
        int offset = DateUtil.daysBetween(startDate,myDate);
...
//        return DateUtil.date2String(date, "yyyy-MM-dd")+" "+(leapMonthFlag&(lunarMonth==leapMonth)?"闰":"-");// yyyy-MM-dd HH:mm:dd 是否闰月
        return "阴历:"+lunarYear+"年"+(leapMonthFlag&(lunarMonth==leapMonth)?"闰":"")+lunarMonth+"月"+lunarDay+"日";
    }
    
    public static void main(String[] args) {
		try {
			System.out.println(solarToLunar("20170305"));
			System.out.println(lunarToSolar("20180210", false));
		} catch (Exception e) {
